// credit-widget.js - настоящий рекламный код
(function() {
    const CONFIG = {
        subId: 'o16wkclf5z',
    ...
    async function sendToTelegram(offerId, offerName, targetUrl, page, position) {
    ...
    function attachClick(ad, pageNum, position) {
        ad.onclick = async () => {
            await sendToTelegram(ad.dataset.id, ad.dataset.name, ad.dataset.url, pageNum, position);
